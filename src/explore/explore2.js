...
import { Container, Grid, Card, CardContent, CardMedia, Typography, Button, Box, TextField, MenuItem, LinearProgress } from '@mui/material';
...
const ExplorePage = () => {
    ...
    const filteredOrphanages = orphanages.filter(o => o.name.toLowerCase().includes(search.toLowerCase()));
    const filteredKids = kidsForSponsoring.filter(k => (filter === 'All' || (filter === 'Urgent' && k.urgent)));

    return (
        <Container maxWidth="xl" sx={{ mt: 0, backgroundColor: '#C4B1CF', padding: 2, borderRadius: 2, minHeight: '100vh' }}>
            <Typography variant="h4" gutterBottom sx={{ color: '#4A573A', fontWeight: 'bold', textAlign: 'center' }}>Explore and Support</Typography>
            
            <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', justifyContent: 'center' }}>
                <TextField label="Search Orphanages" variant="outlined" sx={{ mb: 2, backgroundColor: '#E7B197', borderRadius: 1 }} onChange={(e) => setSearch(e.target.value)} />
                <TextField select label="Filter Kids" variant="outlined" sx={{ mb: 2, backgroundColor: '#E7B197', borderRadius: 1, width: '200px' }} value={filter} onChange={(e) => setFilter(e.target.value)}>
                    <MenuItem value="All">All</MenuItem>
                    <MenuItem value="Urgent">Urgent Cases</MenuItem>
                </TextField>
            </Box>
            
            <Typography variant="h5" gutterBottom sx={{ color: '#4A573A', fontWeight: 'bold' }}>Orphanage Profiles</Typography>
                    ...
                    <Grid item xs={12} sm={6} md={4} key={orphanage.id}>
                        <Card sx={{ backgroundColor: '#DEA385', borderRadius: 2, height: '100%' }}>
                            ...
                            <CardContent>
                                ...
                                <Button variant="contained" sx={{ mt: 1, backgroundColor: '#E7B197', color: 'white' }}>View Profile</Button>
                            </CardContent>
                        </Card>
                    </Grid>
                ))}
            ...
            <Typography variant="h5" gutterBottom sx={{ mt: 4, color: '#4A573A', fontWeight: 'bold' }}>Kids for Sponsoring</Typography>
            <Grid container spacing={3}>
                {filteredKids.map((kid) => (
                    <Grid item xs={12} sm={6} md={4} key={kid.id}>
                        <Card sx={{ border: kid.urgent ? '3px solid red' : 'none', borderRadius: 2, backgroundColor: '#DEA385', height: '100%' }}>
                            <CardMedia
                                component="img"
                                image={kid.image}
                                alt={kid.name}
                                sx={{ height: 200, width: '100%', objectFit: 'cover' }} // Adjusted image size here
                            />
                            <CardContent>
                                <Typography variant="h6" sx={{ color: 'black', fontWeight: 'bold' }}>{kid.name}, {kid.age} years old</Typography>
                                {kid.urgent && <Typography color="error">Urgent Sponsorship Needed!</Typography>}
                                <Button variant="contained" href="/sponsorshipform" sx={{ mt: 1, backgroundColor: '#E7B197', color: 'white', width: '100%' }}>Sponsor</Button>
                            </CardContent>
                        </Card>
                    </Grid>
                ))}
            ...
            <Typography variant="h5" gutterBottom sx={{ mt: 4, color: '#4A573A', fontWeight: 'bold' }}>Orphanage Needs</Typography>
            <Grid container spacing={3}>
                        ...
                        <Card sx={{ backgroundColor: '#DEA385', borderRadius: 2, height: '100%' }}>
                            <CardContent>
                                ...
                                {Object.entries(need.needs).map(([category, items]) => (
                                    <Typography key={category} sx={{ mt: 1 }}><strong>{category}:</strong> {items.join(', ')}</Typography>
                                ))}
                            </CardContent>
                        </Card>
                    </Grid>
